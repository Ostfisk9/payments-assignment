...
RSpec.describe PriceHistory do
  ...
  it "supports filtering on municipality" do
    ...
    history = PriceHistory.call(package: basic, year: "2020", municipality: "Göteborg")
    expect(history).to eq(
      "Göteborg" => [100_00],
    )

    history = PriceHistory.call(package: basic, year: "2020", municipality: "Invalid")
    expect(history).to eq(nil)
  end
end
